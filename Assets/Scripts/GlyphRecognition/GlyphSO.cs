...
public class GlyphSOEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        height += 50;
        bool trackBTN = GUI.Button(new Rect(0, height, Screen.width, 50), "Track Mouse");
        ...
        if (trackBTN)
        {
            if (EditorApplication.isPlaying)
            {
                ...
                Debug.LogWarning("Must be in Play Mode to track mouse!");
        }
    ...
    void NormalisePoints()
    {
        ...
        for (int n = 0; n < glyphSO.points.Length; n++)
        {
            ...
            glyphSO.points[n] = new Vector2(Mathf.Round(newPoint.x * 100) / 100, Mathf.Round(newPoint.y * 100) / 100);
        }
    }
    void CaptureMouse(SceneView sceneView)
    {
        Event e = Event.current;

        if (e.type == EventType.MouseMove && capture)
        {
            Vector2 pos = HandleUtility.GUIPointToScreenPixelCoordinate(e.mousePosition);

            if (glyphPoints.Count == 0 || Vector2.Distance(glyphPoints[glyphPoints.Count - 1], pos) > 5f)
            {
                glyphPoints.Add(pos);
            }
        }
        ...
        if (e.type == EventType.MouseUp && capture)
        {
            ...
            glyphSO.points = glyphPoints.ToArray();
            glyphPoints.Clear();

            EditorUtility.SetDirty(glyphSO);
